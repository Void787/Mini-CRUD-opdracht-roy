<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>hoofpagina</title>
  <link rel="stylesheet" href="main.css">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"
    integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.1/font/bootstrap-icons.css">
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous">
  </script>
</head>

<body>
  <nav>
    <ul class="nav nav-pills nav-fill">
      <li class="nav-item">
        <a class="nav-link hover1" href="Menu.php">Menu</a>
      </li>
      <li class="nav-item login">
        <a class="nav-link hover2" href="login.php">Login</a>
      </li>
      <li class="nav-item">
        <a class="nav-link hover3" href="contact.php">Contact</a>
      </li>
      <img src="img/pizza.png" alt="het is een pizza" width="50" height="50">
      <!--ik moest de lengte en breedte van de foto hier neerzetten want hij deed het niet als ik het in de css zette.-->
    </ul>
  </nav>
  <main>
    <div class="card text-white bg-warning mb-3 cv1">
    <img class="card-img cv1" src="img/pizza.jpg" alt="Card image">
    <div class="card-img-overlay">
      <h5 class="card-title text-center">welkom bij Pizzahuis</h5>
      <p class="card-text text-center"></p>
      <p class="card-text text-center"></p>
    </div>
    </div>
  </main>
</body>
</html>